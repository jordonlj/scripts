...
#git_logs_local.py fetch -u jason.liu@linaro.org -p <my password>
#git_logs_local.py analyze --since 20150818 --until 20150819
...
#scp jason.liu@weblogs.linaro.org:/home/jason.liu/image.png ./
...
def _analyze(args):
    ...
    #Date--------------------------------
    items = dic_date.items() 
    items.sort()
    for (k,v) in items:
        log.info('analyzing date: %s, hits: %d', k, v)
        keyList.append(k)
        valueList.append(v)

    n_groups = len(dic_date)
    means_created = valueList
    index = np.arange(n_groups)
    opacity = 0.6

    rects = plt.bar(index, means_created, 1, alpha=opacity, color='b',label='Clone & Fetch Activities')
